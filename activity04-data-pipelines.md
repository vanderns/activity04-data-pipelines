...
**Response**: Shows output that code loaded
...
**Response**: We need majors and unemployment rates that go with them.
In Python I would attempt to use major\[np.argmin(unemployment\_rate)\]
and in SAS I could use proc sort by unemployment\_rate desc;
...
**Response**: Mathematics And Computer Science has the lowest
unemployment rate
...
``` r
college_recent_grads %>% 
  filter(major %in% stem_categories) %>% 
...
```
    ...
    ##   <chr>             <dbl>  <dbl> <dbl>
    ## 1 Physical Sciences 30000  40000 55000

Discuss your output as it relates to the research question.

**Response**: Well Physical Sciences is the only STEM major in there and
it makes over the median
